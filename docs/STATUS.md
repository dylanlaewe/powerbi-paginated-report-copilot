...
The first natural-language copilot MVP flow is accepted. Independent Windows testing passed checksum, open, Design, requested title/data, Preview, hierarchy, Region subtotals, Grand Total, pagination, repeated headings, Page N of M, no blank pages or clipping, PDF, and Excel. Numeric page and worksheet counts were not provided. Minimal Electron integration provides controlled local generation, structured results, visible errors, Finder reveal, and path copy. After fixing the preload and cwd-dependent resource lookup, an automated Mac renderer interaction generated the canonical report through the actual bridge and UI; its SHA and bytes matched the accepted CLI artifact. Independent user retest remains pending.
