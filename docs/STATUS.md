...
Candidates 01 and 02 remain accepted. Candidate 03 is rejected: despite passing XML, XSD, and static consistency checks, it failed during Report Builder open with an index-out-of-range exception before Design or Preview. Its hierarchy will not be modified further by guesswork. The next gate must derive Candidate 03b from the accepted Report Builder-authored `KnownGoodRegionGroup.rdl` structure. The branch remains unmerged and untagged.
