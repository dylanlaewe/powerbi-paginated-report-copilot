...
Candidate 01 is accepted after independent Windows validation. It opened without repair/conversion, loaded Design view, recognized `SeedData`, previewed all three Region/Revenue rows, supported the inherited expand/collapse interaction, and rendered the execution-time footer. The accepted file remains SHA-256 pinned and unchanged. The spike remains unmerged while Candidate 02 adds only detail columns and six synthetic rows on the validated structure.
