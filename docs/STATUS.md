...
Candidates 01 and 02 remain accepted; Candidate 03 is rejected and preserved. Three-way forensics show that Report Builder places the Region header in a two-row, nested `Region → Region1 → Details` row hierarchy, whereas Candidate 03 used a third merged body row and `Region → Details`. Both are count-consistent, so no root cause is asserted. Regression validation now distinguishes the accepted grouped structure. Candidate 03b must be an exact grouped-seed derivative; the branch remains unmerged and untagged.
